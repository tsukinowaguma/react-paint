...
export default class Graffiti extends Component{
	...
	render(){
		let color = ["black","white","green","blue","red"];
		let lineWidth = [2,3,5,7];
		let width,height;
		if(this.props.width && this.props.width>=250) width = this.props.width;
		else width = 300;
		if(this.props.height && this.props.height>=250) height = this.props.height;
		else height = 300;
		return(
			<div className="graffiti" style={{width:width+'px',height:height+'px'}}>
				{
					this.state.isImg ? (
						<div>
									...
									color.map((item) => {
										return this.state.strokeColor== item?
											<li style={{backgroundColor:item}} data-color={item} 
												className='color-current'></li> :
											<li style={{backgroundColor:item}} data-color={item} ></li>
									})
								}
							</ul>
									...
									lineWidth.map((item) => {
										return this.state.lineWidth == item?
											<li style={{width:item*3+'px',height:item*3+'px'}} data-value={item} 
												className='width-current'></li> :
											<li style={{width:item*3+'px',height:item*3+'px'}} data-value={item} ></li>									})
								}
							</ul>
